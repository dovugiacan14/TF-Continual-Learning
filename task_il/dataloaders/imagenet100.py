...
def get(seed=42,pc_valid=0, inc=5):
    ...
    data_dir = os.path.join(project_dir, 'dat', 'binary_imagenet_inc%d' % inc)
    ...
    if need_download:
        ...
        dat['train'] = datasets.ImageFolder('/path/to/Imagenet/train', transform=trsf)
        dat['test'] = datasets.ImageFolder('/path/to/Imagenet/val', transform=trsf)

        for n in range(task_num):
            data[n]={}
            data[n]['name']='imagenet100'
            data[n]['ncla']=0
            data[n]['train']={'x': [],'y': []}
            data[n]['test']={'x': [],'y': []}
        for s in ['train','test']:
