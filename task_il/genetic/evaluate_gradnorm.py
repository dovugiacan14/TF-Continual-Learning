"""
GradNorm-based Fitness Evaluation
Uses Gradient Norm (L2 norm of gradients) instead of training for fast architecture evaluation.
Adapted from ZenNAS/ZeroShotProxy/compute_gradnorm_score.py
"""
...
class GradNormEvaluate(object):
    """
    Evaluate fitness using GradNorm score instead of training.
    Uses random data and random labels - no real dataset needed.
    """
    ...
    def generate_to_python_file(self):
        """Generate Python files using gradnorm template"""
        self.log.info('Begin to generate gradnorm python files')
        for indi in self.individuals:
            Utils.generate_gradnorm_file(indi)
        self.log.info('Finish the generation of gradnorm python files')

    def evaluate(self):
        """
        Evaluate fitness using GradNorm score.
        Runs sequentially (no multiprocessing) for CUDA compatibility.
        """
        self.log.info('Query gradnorm fitness from cache')
        ...
        for indi in self.individuals:
            ...
            if _key in _map:
                _count += 1
                _score = _map[_key]
                self.log.info('Hit the cache for %s, key:%s, gradnorm:%.5f, assigned_score:%.5f'%
                            (indi.id, _key, float(_score), indi.acc))
                indi.acc = float(_score)
        self.log.info('Total hit %d individuals for gradnorm fitness'%(_count))
        ...
        for indi in self.individuals:
            if indi.acc < 0:
                ...
                if gpu_id is not None:
                    file_name = indi.id
                    self.log.info('Begin to evaluate gradnorm for %s'%(file_name))
                    ...
                    self.log.info('Finished gradnorm evaluation for %s'%(file_name))
            else:
                file_name = indi.id
                self.log.info('%s has inherited the gradnorm fitness as %.5f, no need to evaluate'%
                            (file_name, indi.acc))
                f = open('./populations/after_%s.txt'%(file_name[4:6]), 'a+')
                # Write gradnorm format
                f.write('%s={gradnorm:%.5f, avg:0.0, std:0.0, params:0.0}\n'%(file_name, indi.acc))
                f.flush()
                f.close()
        ...
        if has_evaluated_offspring:
            ...
            # Load gradnorm fitness from files
            file_name = './populations/after_%s.txt'%(self.individuals[0].id[4:6])
            assert os.path.exists(file_name) is True
            f = open(file_name, 'r')
            fitness_map = {}
            for line in f:
                if len(line.strip()) > 0:
                    # GradNorm format: indiXXXX={gradnorm:73.074, avg:..., std:..., params:...}
                    parts = line.strip().split('=')
                    indi_id = parts[0]

                    if '{' in parts[1]:
                        # Parse gradnorm metrics dict
                        metrics_str = parts[1]
                        metrics_str = metrics_str.replace('{', '').replace('}', '')
                        metrics_parts = metrics_str.split(',')
                        # Extract gradnorm score
            ...
            self.log.info('None offspring has been evaluated with gradnorm')
