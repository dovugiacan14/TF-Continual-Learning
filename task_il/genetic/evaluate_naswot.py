"""
NASWOT Fitness Evaluation
Uses NASWOT score (log-determinant of activation kernel matrix) instead of training
for fast architecture evaluation.
...
class NaswotEvaluate(object):
    """
    Evaluate fitness using NASWOT score instead of training.
    Based on Mellor et al. (ICML 2021) - measures network expressivity
    via log-determinant of the ReLU activation kernel matrix.
    """
    ...
    def generate_to_python_file(self):
        """Generate Python files using naswot template"""
        self.log.info('Begin to generate naswot python files')
        for indi in self.individuals:
            Utils.generate_naswot_file(indi)
        self.log.info('Finish the generation of naswot python files')

    def evaluate(self):
        """
        Evaluate fitness using NASWOT score.
        Runs sequentially (no multiprocessing) for CUDA compatibility.
        """
        self.log.info('Query naswot fitness from cache')
        ...
        for indi in self.individuals:
            ...
            if _key in _map:
                _count += 1
                _score = _map[_key]
                self.log.info('Hit the cache for %s, key:%s, naswot:%.5f, assigned_score:%.5f'%
                            (indi.id, _key, float(_score), indi.acc))
                indi.acc = float(_score)
        self.log.info('Total hit %d individuals for naswot fitness'%(_count))
        ...
        for indi in self.individuals:
            if indi.acc < 0:
                ...
                if gpu_id is not None:
                    file_name = indi.id
                    self.log.info('Begin to evaluate naswot for %s'%(file_name))
                    ...
                    self.log.info('Finished naswot evaluation for %s'%(file_name))
            else:
                file_name = indi.id
                self.log.info('%s has inherited the naswot fitness as %.5f, no need to evaluate'%
                            (file_name, indi.acc))
                f = open('./populations/after_%s.txt'%(file_name[4:6]), 'a+')
                # Write naswot format
                f.write('%s={naswot:%.5f, avg:0.0, std:0.0, params:0.0}\n'%(file_name, indi.acc))
                f.flush()
                f.close()
        ...
        if has_evaluated_offspring:
            ...
            # Load naswot fitness from files
            file_name = './populations/after_%s.txt'%(self.individuals[0].id[4:6])
            assert os.path.exists(file_name) is True
            f = open(file_name, 'r')
            fitness_map = {}
            for line in f:
                if len(line.strip()) > 0:
                    # NASWOT format: indiXXXX={naswot:73.074, avg:..., std:..., params:...}
                    parts = line.strip().split('=')
                    indi_id = parts[0]

                    if '{' in parts[1]:
                        # Parse naswot metrics dict
                        metrics_str = parts[1]
                        metrics_str = metrics_str.replace('{', '').replace('}', '')
                        metrics_parts = metrics_str.split(',')
                        # Extract naswot score
            ...
            self.log.info('None offspring has been evaluated with naswot')
