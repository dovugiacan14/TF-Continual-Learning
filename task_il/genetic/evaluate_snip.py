"""
SNIP (Single-shot Network Pruning) Fitness Evaluation
Uses connection sensitivity instead of training for fast architecture evaluation.
...
class SnipEvaluate(object):
    """
    Evaluate fitness using SNIP score instead of training.
    ...
    def generate_to_python_file(self):
        """Generate Python files using snip template"""
        self.log.info('Begin to generate snip python files')
        for indi in self.individuals:
            Utils.generate_snip_file(indi)
        self.log.info('Finish the generation of snip python files')

    def evaluate(self):
        """
        Evaluate fitness using SNIP score.
        Runs sequentially (no multiprocessing) for CUDA compatibility.
        """
        self.log.info('Query snip fitness from cache')
        ...
        for indi in self.individuals:
            ...
            if _key in _map:
                _count += 1
                _score = _map[_key]
                self.log.info('Hit the cache for %s, key:%s, snip:%.5f, assigned_score:%.5f'%
                            (indi.id, _key, float(_score), indi.acc))
                indi.acc = float(_score)
        self.log.info('Total hit %d individuals for snip fitness'%(_count))
        ...
        for indi in self.individuals:
            if indi.acc < 0:
                ...
                if gpu_id is not None:
                    file_name = indi.id
                    self.log.info('Begin to evaluate snip for %s'%(file_name))
                    ...
                    self.log.info('Finished snip evaluation for %s'%(file_name))
            else:
                file_name = indi.id
                self.log.info('%s has inherited the snip fitness as %.5f, no need to evaluate'%
                            (file_name, indi.acc))
                f = open('./populations/after_%s.txt'%(file_name[4:6]), 'a+')
                # Write snip format
                f.write('%s={snip:%.6f, raw:0.0, std:0.0, params:0.0}\n'%(file_name, indi.acc))
                f.flush()
                f.close()
        ...
        if has_evaluated_offspring:
            ...
            for line in f:
                if len(line.strip()) > 0:
                    # SNIP format: indiXXXX={snip:5.123456, raw:..., std:..., params:...}
                    parts = line.strip().split('=')
                    indi_id = parts[0]

                    if '{' in parts[1]:
                        # Parse snip metrics dict
                        metrics_str = parts[1]
            ...
            self.log.info('None offspring has been evaluated with snip')
