"""
GraSP-based Fitness Evaluation
Uses Gradient Signal Preservation (GraSP) instead of training for fast architecture evaluation.
...
class GraSPEvaluate(object):
    """
    Evaluate fitness using GraSP score instead of training.
    Uses real CIFAR-100 data for gradient and Hessian-gradient product computation.
    """
    ...
    def generate_to_python_file(self):
        """Generate Python files using grasp template"""
        self.log.info('Begin to generate grasp python files')
        for indi in self.individuals:
            Utils.generate_grasp_file(indi)
        self.log.info('Finish the generation of grasp python files')

    def evaluate(self):
        """
        Evaluate fitness using GraSP score.
        Runs sequentially (no multiprocessing) for CUDA compatibility.
        """
        self.log.info('Query grasp fitness from cache')
        ...
        for indi in self.individuals:
            ...
            if _key in _map:
                _count += 1
                _score = _map[_key]
                self.log.info('Hit the cache for %s, key:%s, grasp:%.5f, assigned_score:%.5f'%
                            (indi.id, _key, float(_score), indi.acc))
                indi.acc = float(_score)
        self.log.info('Total hit %d individuals for grasp fitness'%(_count))
        ...
        for indi in self.individuals:
            if indi.acc < 0:
                ...
                if gpu_id is not None:
                    file_name = indi.id
                    self.log.info('Begin to evaluate grasp for %s'%(file_name))
                    ...
                    self.log.info('Finished grasp evaluation for %s'%(file_name))
            else:
                file_name = indi.id
                self.log.info('%s has inherited the grasp fitness as %.5f, no need to evaluate'%
                            (file_name, indi.acc))
                f = open('./populations/after_%s.txt'%(file_name[4:6]), 'a+')
                f.write('%s={grasp:%.5f, raw:0.0, std:0.0, params:0.0}\n'%(file_name, indi.acc))
                f.flush()
                f.close()
        ...
        if has_evaluated_offspring:
            ...
            for line in f:
                if len(line.strip()) > 0:
                    # GraSP format: indiXXXX={grasp:73.074, raw:..., std:..., params:...}
            ...
            self.log.info('None offspring has been evaluated with grasp')
