"""
TASS (Task-Aware Sensitivity Score) Fitness Evaluation
Training-free proxy designed specifically for Continual Learning NAS.
Combines expressivity (Zen), feature diversity (Rank), and interference measurement.
"""
...
class TASSEvaluate(object):
    """
    Evaluate fitness using TASS instead of training.
    ...
    def generate_to_python_file(self):
        """Generate Python files using TASS template"""
        self.log.info('Begin to generate tass python files')
        for indi in self.individuals:
            Utils.generate_tass_file(indi)
        self.log.info('Finish the generation of tass python files')

    def evaluate(self):
        """
        Evaluate fitness using TASS score.
        Runs sequentially (no multiprocessing) for CUDA compatibility.
        """
        self.log.info('Query tass fitness from cache')
        ...
        for indi in self.individuals:
            ...
            if _key in _map:
                _count += 1
                _score = _map[_key]
                self.log.info('Hit the cache for %s, key:%s, tass:%.5f, assigned_score:%.5f'%
                            (indi.id, _key, float(_score), indi.acc))
                indi.acc = float(_score)
        self.log.info('Total hit %d individuals for tass fitness'%(_count))
        ...
        for indi in self.individuals:
            if indi.acc < 0:
                ...
                if gpu_id is not None:
                    file_name = indi.id
                    self.log.info('Begin to evaluate tass for %s'%(file_name))
                    ...
                    self.log.info('Finished tass evaluation for %s'%(file_name))
            else:
                file_name = indi.id
                self.log.info('%s has inherited the tass fitness as %.5f, no need to evaluate'%
                            (file_name, indi.acc))
                f = open('./populations/after_%s.txt'%(file_name[4:6]), 'a+')
                f.write('%s={tass:%.5f, zen:0.0, rank:0.0, itf:0.0, params:0.0}\n'%(file_name, indi.acc))
                f.flush()
                f.close()
        ...
        if has_evaluated_offspring:
            ...
            for line in f:
                if len(line.strip()) > 0:
                    # TASS format: indiXXXX={tass:73.074, zen:..., rank:..., itf:..., params:...}
            ...
            self.log.info('None offspring has been evaluated with tass')
