...
class EWC(BaseLearner):
    ...
    def incremental_train(self, data_manager):
        ...
        if self.fisher is None:
            ...
            for n, p in self.fisher.items():
                if n in new_finsher.keys():
                    self.fisher[n] = (
                        alpha * self.fisher[n]
                        + (1 - alpha) * new_finsher[n][: len(self.fisher[n])]
                    )
                    if len(new_finsher[n]) > len(self.fisher[n]):
                        new_finsher[n][: len(self.fisher[n])] = (
                            alpha * self.fisher[n]
                            + (1 - alpha) * new_finsher[n][: len(self.fisher[n])]
                        )
                        self.fisher[n] = copy.deepcopy(new_finsher[n])
            """
    ...
    def compute_ewc(self):
        ...
        if len(self._multiple_gpus) > 1:
            ...
            for n, p in self._network.named_parameters():
                if n in self.fisher.keys() and n in self.mean.keys():
                    loss += (
                        torch.sum(
                            (self.fisher[n])
                            * (p[: len(self.mean[n])] - self.mean[n]).pow(2)
